...
中身、つまりどの問題を何と比べてどれだけ一致したかをまとめたものです。ソルバの概要や
...
軸対称のベンチマークです。圧力場は解析的な接触円を満たし、すべての格子セルの圧力を $`r/a`$ に
...
Hertz と違って、圧力は尖点で対数的に発散します。そのため、メッシュに依存するピークそのもの
は比べません。一方で、半径方向のプロファイルは Sneddon の閉形式に従います：
...
（高さ場の足し算）と、単一の Hertz 円板が突起接触の格子へと分裂します。同じ荷重のもとでも、
...
保存するので、それ自体が検証にもなります。各フランクは荷重の半分を担う楕円 Hertz 接触なので、
...
この数値は、フランク圧がギャラリーの GPa 域に収まるように選びました
...
重なり領域には閉形式がありません。2つのフランクが弾性場を通じて相互作用し、荷重がきれいには
$`P/2`$ ずつに分かれないからです。重なりはピーク圧を、分離時の $`(1/2)^{1/3}`$ 倍の値より
...
- **パネル C — 除荷フランク。** 除荷するフランクは普遍曲線 $`Q_-/Q_-(0) = (1-\xi)^{3/2}`$ に
  従って接線的にゼロへ近づきます。ソルバの非対称な井戸の実験で得たマーカーが、3 % 以内でその上に
  乗ります。3/2 乗が、そのまま `C¹` を生むわけです。
...
を係数に還元して純粋関数の法則に差し込み、既定で場ソルバに対する自己検証を走らせます。
...
両立が一目で確認できます。使い方は [README](../README.md) の「較正パイプライン」の節を、固定は
...
- **パネル B — 半重なりの断面。** 厳密解（ソルバ、橙）と軽量式を直接並べます。素朴な和は
  継ぎ目を約 +68 % のスパイクに積み上げますが、包絡 $`\max(p_+, p_-)`$（紫）はソルバに
  −4 % で乗り、連結したサドルを再現します。
...
等分し、面圧ピークが 1:1 の場合でした。けれども、クーロン摩擦が効くのは玉が溝を横切って
引きずられるときで、横断ドライブが荷重を片方のフランクへ寄せ、2つの面圧ピークは開きます。
本タスクでは形状はそのままに、横断ドライブだけを与えて面圧ピークが 2:1 になるところまで
寄せました。配置はフランクが干渉するもの、すなわち半重なりのシム $`y_0 = b/2`$ です（2つの
footprint が交差してひと続きの連結パッチになる、フランク干渉の基本配置）。場ソルバでは横断変位を、
子午線方向の井戸床オフセット $`df`$ として与えます（2つのフランク井戸の片方を $`df`$ だけ持ち上げ、
近い側のフランクをそれだけ深く押す、玉の横変位を高さ場で裏返したものです。ギャップは2トーラスの
点ごとの最小のままです）。半重なり（$`y_0 = b/2 \approx 0.51\,\mathrm{mm}`$）では
$`df \approx 5.7\,\mu\mathrm{m}`$ が面圧ピーク比 $`p_+ : p_- = 2.0 : 1`$（$`p_+ \approx 2.15\,\mathrm{GPa}`$、
$`p_- \approx 1.07\,\mathrm{GPa}`$）を生みます。2つのフランク接触は重なっており
...
- **(A) 2:1 への駆動 — 分割の予測。** 横断ドライブ $`df/\delta_0`$ を上げると、2トーラスのピーク比
  $`p_+/p_-`$ が 1:1 から 2:1 へ上がります。各フランクは自分の荷重 $`Q`$ を担う楕円 Hertz パッチで、ピークは
  立方根則 $`p_0 = c_p Q^{1/3}`$ に乗るので、2:1 のピーク比はちょうど 8:1 の荷重分割になります。軽量
  カップリング則の予測 $`(Q_+/Q_-)^{1/3}`$ がソルバ点を 2.5 % 以内で追います。
- **(B) 2:1 干渉断面 — 厳密 vs 軽量。** 半重なり 2:1 ドライブの子午線断面で、ソルバ（厳密、橙点）・
  軽量包絡 $`\max(p_+, p_-)`$（紫）・素朴な和（赤破線）を並べます。2つのクレストはきれいな 2:1
  （$`p_+ \approx 2.15`$、$`p_- \approx 1.07\,\mathrm{GPa}`$）で、間を荷重を担う連結サドルがつなぎます
  （フランクが干渉してひと続きになっている証拠です）。素朴な和は重なりをサドルで二重計上し、サドルで
  約 +135 % も過大評価しますが、包絡はソルバピークに −1.0 % で乗り、連結サドルを再現します。
  網掛けは片寄ったクーロンキャップ $`\mu p`$ で、引きずられた側のフランクがトラクションを担います。
- **(C) 2次元クーロンキャップ（連結）。** $`\mu p(x,y)`$ を2つの重なった不等なフランク楕円
  （ひと続きの連結パッチ）として描き、ソルバの連結接触縁を重ねます。大きな主パッチ
  （$`\mu Q_+ \approx 89\,\mathrm{N}`$）と小さな隣パッチ（$`\mu Q_- \approx 11\,\mathrm{N}`$）で、各々が
  $`\mu Q`$ の全滑り摩擦に積分されます。
- **(D) シムを詰めて重なりへ。** 横断ドライブを固定したままフランクオフセット $`y_0/b`$ を詰めて分離から
  重なりへ入れると、連結サドルが立ち上がります。素朴な和（赤破線）はサドルを二重計上して過大
  評価する一方、包絡（紫）はソルバ（橙）のサドルを追います。これが「和ではなく包絡」がフランク干渉の
  もとで効く理由です。
...
（`test_asymmetric_gothic_flanks_cap_a_two_to_one_peak`）で固定しています。なお半重なりでは、軽量
包絡は浅い側フランクを幾何オフセット $`\pm y_0`$ に置くため、そのクレスト位置は、重なりで外側へずれる
実際の荷重重心（前節の二次の方向効果）とわずかにずれます。支配クレストと連結サドルは捉えますが、
位置の補正は、単一アーチへの完全合体とあわせて次の段階に回します。
