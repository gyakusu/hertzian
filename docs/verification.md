...
ときで、横断ドライブが荷重を片方のフランクへ寄せ、2つの面圧ピークは開きます。本タスクでは
**形状はそのまま**——しかも**フランクが干渉する**配置、すなわち**半重なり**のシム $y_0 = b/2$
（2つの footprint が交差して**ひと続きの連結パッチ**になる、フランク干渉の基本配置）——に、横断
ドライブだけを与え、面圧ピークが**2:1**になるところまで寄せたのが本タスクの解析です。場ソルバでは
横断変位を子午線方向の**井戸床オフセット** $df$——2つのフランク井戸の片方を $df$ だけ持ち上げ、
近い側のフランクをそれだけ深く押す、玉の横変位の高さ場での裏返し——として与えます（ギャップは
2トーラスの点ごとの最小のまま）。半重なり（$y_0 = b/2 \approx 0.51\,\mathrm{mm}$）で
$df \approx 5.7\,\mu\mathrm{m}$ が面圧ピーク比 $p_+ : p_- = 2.0 : 1$（$p_+ \approx 2.15\,\mathrm{GPa}$、
$p_- \approx 1.07\,\mathrm{GPa}$）を生みます。2つのフランク接触は**重なって**おり
（`separated = False`）、間を**荷重を担うサドル**（$\approx 0.85\,\mathrm{GPa}$、浅い側クレストの約 79 %）が
つなぐ、ひと続きの連結パッチです。
...
- **(A) 2:1 への駆動 — 分割の予測。** 横断ドライブ $df/\delta_0$ を上げると、2トーラスのピーク比
  ...
  立方根則 $p_0 = c_p Q^{1/3}$ に乗るので、**2:1 のピーク比はちょうど 8:1 の荷重分割**です。軽量
  カップリング則の予測 $(Q_+/Q_-)^{1/3}$ がソルバ点を **2.5 % 以内**で追います。
- **(B) 2:1 干渉断面 — 厳密 vs 軽量。** 半重なり 2:1 ドライブの子午線断面で、ソルバ（厳密、橙点）・
  軽量包絡 $\max(p_+, p_-)$（紫）・素朴な和（赤破線）を並べます。2つのクレストはきれいな 2:1
  （$p_+ \approx 2.15$、$p_- \approx 1.07\,\mathrm{GPa}$）で、間を**荷重を担う連結サドル**がつなぐ
  （フランクが干渉してひと続きになっている証拠）。**素朴な和は重なりをサドルで二重計上し、サドルで
  約 +135 % も過大評価**する一方、**包絡はソルバピークに −1.0 % で乗り**、連結サドルを再現します。
  網掛けは片寄ったクーロンキャップ $\mu p$——引きずられた側のフランクがトラクションを担います。
- **(C) 2次元クーロンキャップ（連結）。** $\mu p(x,y)$ を2つの**重なった不等な**フランク楕円
...
2:1 の面圧ピーク（立方根則による ~8:1 荷重分割）、フランクが**重なって連結**すること（中央が荷重を
担い、サドルが浅い側クレストの下に立つ）、包絡クレストがソルバピークに乗り主フランクと一致すること、
...
包絡は浅い側フランクを幾何オフセット $\pm y_0$ に置くため、その**クレスト位置**は重なりで外側へずれる
実際の荷重重心（前節の二次の方向効果）とわずかにずれます——支配クレストと連結サドルは捉えますが、
位置の補正は単一アーチへの完全合体とあわせて次の段階です。
