...
**非対称ドライブ — 2トーラスで 2:1 の面圧ピーク。** ここまでは玉を溝へ**まっすぐ**押し込む
対称な押し込みで、2つのフランクは荷重を等分し、面圧ピークは 1:1 でした。けれどもクーロン摩擦が
効くのは、玉が溝を**横切って引きずられる**ときです。横断ドライブが荷重を片方のフランクへ寄せ、
2つの面圧ピークは開きます。これがまさにキャップの出番——合力だけでなく、どちらのフランクが
どれだけのトラクションを担えるかが要るからです。**形状はそのまま**（2トーラスのフランク干渉
モデル）に、横断ドライブだけを与えて面圧ピークが**2:1**になるところまで寄せます。場ソルバでは
横断変位を、子午線方向の**井戸床オフセット** $df$——2つのフランク井戸の片方を $df$ だけ持ち上げ、
近い側のフランクをそれだけ深く押す、玉の横変位の高さ場での裏返し——として与えます（形状は
2トーラスの点ごとの最小のまま）。各フランクは依然として荷重 $Q$ を担う楕円 Hertz パッチで、ピークは
立方根則 $p_0 = c_p Q^{1/3}$ に乗るので、**2:1 のピーク比はちょうど 8:1 の荷重分割**です（深い側が
約 8 倍）。場ソルバ（厳密）と軽量式（包絡）を並べると、非対称な 2:1 でも軽量式は厳密解にほぼ
重なります。分割の予測も、軽量カップリング則が場ソルバを 2:1 まで追います。詳しい一致は
