...
（$M \propto Q^{4/3}\to 0$）ので、面圧像は力則の `C¹` な 2→1 受け渡しを継ぎ目なく追います。

#### 重なり — 連結接触は面圧の pointwise max（ギャップの min の双対）

力則が半重なり域までカバーするのと同じく、面圧分布も **2 フランクが重なる場合**を扱います。シムを詰めて
2 つのフランク接触が**半分重なる**と、接触は 1 つの**連結したパッチ**になり、2 つのピークの間に**サドル**
（荷重を担うゴシック点）が現れます——分離アーチの非接触リッジとは対照的です。

ここで**組み方が効きます**。2 つの半楕円体を**足す**のは質的に誤りで、重なり帯を二重計上して**フランク
ピークより高い中央の山**を作りますが、実際の連結接触はピークより**低いサドル**を持ちます（弾性相互作用は
重なりで圧を*盛る*のではなく*逃がす*）。正しい軽量則は**点ごとの最大**

$$p(x, y) = \max\bigl(p_+(x,\, y - y_0),\; p_-(x,\, y + y_0)\bigr)$$

（`FlankPressure.two_flank_pressure_at`）で、各点で**近い（荷重の大きい）フランクが勝ちます**。これは
ゴシック**ギャップ**が 2 つの井戸の点ごとの**最小**（近い*面*が勝つ）であることのちょうど**双対**です
——面圧は近い*フランク*が勝つ。

![縮約面圧の重なり2パネル検証：(A) 半重なりで連結した縮約面圧場（pointwise max）に場ソルバの接触エッジを重ね、2つのローブが1つの連結パッチになる様子；(B) 子午線断面で、ソルバ点が縮約 max（連結サドル）に乗り、素朴な和はフランクピークを超える中央の山という誤った位相を作る様子。](docs/img/pressure_overlap.png)

点ごとの最大は連結サドルを与え、場ソルバの子午線プロファイルに **RMS ~10 %**（和は ~35 %）で追従します。
ピーク（各フランクの荷重 $Q$ から）はソルバと数 % 以内、中央は**連結したサドル**（荷重を担い、ピークより下：
ソルバの $0.76\cdot$ピークに対し max は約 $0.89\cdot$ピーク）です。残差——重なりがピークを数 % 補強し、真の
サドルは幾何学的交差より ~20 % 低い——は力則が次段に残すのと同じ弾性相互作用の留保で、**各フランクの
パッチ寸法・ピーク・スピンモーメントは（カップリング込みの）$Q$ から決まる**ので重なりを越えて一次で正しい
ままです。連結サドル・和との対比・近接フランク優越は Rust・Python テストに固定しています。
...
# 重なる場合は和ではなく点ごとの最大で連結接触を組む（y0 はフランクオフセット）：
p_connected = flank.two_flank_pressure_at(q_plus, q_minus, 0.27e-3, 1e-5, 2e-5)
```
