...
上の例では、フランクの相対半径・オフセット・接触角を**手で**求めて法則に渡していました。
`hertzian.calibrate` はこの還元を自動化します。Rust 側は係数 $(K, \alpha, \kappa)$ を**後から差し込む
だけの純粋関数** $F(\delta)$ であり、Python 側のパイプラインが物理形状（玉・溝・基準中心の各半径、
アーチのシム、弾性係数）から係数を導いて差し込みます。数行で、すぐ使える法則が得られます：
...
`calibrate` は既定で、蒸留元である FFT+BCCG 場ソルバに対して結果を**自己検証**します。単一アーチの
荷重スイープで Hertz の 1.5 乗則と剛性 $K$ を確認し、二フランクの一点で結合後の有効フランク数 $\eta$ を
照合し、力評価の速度を測ります。`describe()` がそれを一目で読める形にまとめます：
...
ここで精度（指数 1.5、$R^2 \approx 1$、残差 0.2 % 未満、$\eta$ の一致 0.1 %）と速度（場ソルバ比で
約 10⁵ 倍）が**両立**していることが確認できます。ソルブを走らせず解析係数だけを即座に得たいときは
`hertzian.calibrate(spec, verify=False)` とします（`cal.verification` は `None`）。実行できる一式は
...
**非対称ドライブ — フランクが干渉する 2トーラスで 2:1 の面圧ピーク。** ここまでは玉を溝へ
**まっすぐ**押し込む対称な押し込みで、2つのフランクは荷重を等分し、面圧ピークは 1:1 でした。
けれどもクーロン摩擦が効くのは、玉が溝を**横切って引きずられる**ときです。横断ドライブが荷重を
片方のフランクへ寄せ、2つの面圧ピークは開きます。本タスクでは**形状はそのまま**——しかも
**フランクが干渉する**配置、すなわち**半重なり**のシム $y_0 = b/2$（2つの footprint が交差して
**ひと続きの連結パッチ**になる、フランク干渉の基本配置。$y = 0$ のゴシック点も荷重を担います）——に、
横断ドライブだけを与えて面圧ピークが**2:1**になるところまで寄せます。場ソルバでは横断変位を、子午線
方向の**井戸床オフセット** $df$——2つのフランク井戸の片方を $df$ だけ持ち上げ、近い側のフランクを
それだけ深く押す、玉の横変位の高さ場での裏返し——として与えます（ギャップは2トーラスの点ごとの最小の
まま）。各フランクは依然として荷重 $Q$ を担う楕円 Hertz パッチで、ピークは立方根則 $p_0 = c_p Q^{1/3}$
に乗るので、**2:1 のピーク比はちょうど 8:1 の荷重分割**です（深い側が約 8 倍）。**重なるので和ではなく
包絡が要ります**——素朴な和は重なりを二重計上して継ぎ目を非物理的なスパイクに積み上げる一方、包絡
$\max(p_+, p_-)$ は2つの 2:1 クレストと、それをつなぐ**荷重を担う連結サドル**を取り戻します。場ソルバ
（厳密）と軽量式を並べると、非対称な 2:1 でも包絡はピークを数 % 以内で追い、連結サドルを再現します
（一方、素朴な和はサドルで大きく過大評価します）。詳しい一致は
